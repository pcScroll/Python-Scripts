...
DocumentPath = r"\\AS001fSC01FS\\gisapplications\\admgis1061\\arcgisdata\\Smartsurvey\\" + TempalteName +  ".mxd"
...
#-----------------------------------------------------------------------------------------------------------
V_APPLICATION_INFO = arcpy.mapping.ListTableViews(mxd,"V_APPLICATION_INFO")[0]
...
for row in pCursor:
        ...
        ENGGINEERNAME = row[56]
        ENGG_FINISH_DATE = row[57]
        RecipientName = None
        Comments = None
        Finalcertifiedname = None
        DEGREE = row[64]
#===========================================================================================================
...
#-----------------------------------------------------------------------------------------------------------
pCursor = arcpy.SearchCursor(r"\\AS001fSC01FS\gisapplications\admgis1061\arcgisdata\Smartsurvey\Layers\S_TSUR_POINT.lyr","REQNUM = '" + REQNUM + "'")
#-----------------------------------------------------------------------------------------------------------
pCoordinates = []
for row in pCursor:
...
for elm in arcpy.mapping.ListLayoutElements(mxd, "TEXT_ELEMENT"):
        ...
        if elm.name == "Comments":
                if Comments is None: elm.text = " "
                else: elm.text = Comments
        if elm.name == "ENGGINEERNAME":
                if ENGGINEERNAME is None: elm.text = " "
                else: elm.text = ENGGINEERNAME
        if elm.name == "ENGG_FINISH_DATE":
                if ENGG_FINISH_DATE is None: elm.text = " "
                else: elm.text = ENGG_FINISH_DATE
        if elm.name == "Finalcertifiedname":
                if Finalcertifiedname is None: elm.text = " "
                else: elm.text = Finalcertifiedname
